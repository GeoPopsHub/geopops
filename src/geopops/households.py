...
from .utils import (PersonData, Household, GQres, Indexer, TraitSchema,
                    tryJSON, resolve_config, thresh, ranges, lrRound)
...
def generate_group_quarters(config, cbgs, cbg_indexer, ind_codes, data_dir, rng,
    ...
    # GQ residents have no PUMS sample behind them, so every trait is None; they
    # still share the population's schema so they stay comparable to households.
    if schema is None:
...
def _person_columns(p_samps, ind_codes, additional_traits):
    """Precompute the per-person values needed to build PersonData, as arrays.

    These used to be computed row by row inside the person loop --- four
    `.apply(axis=1)` passes over the sample frame, plus a `p_samps.iloc[r-1]`
    per person that built a fresh pandas Series just to read a handful of
    scalars out of it. Doing the same work once, column-wise, is far cheaper:
    the loop then only indexes plain numpy arrays.

    `com_cat` and `com_inc` use -1 rather than None for "not applicable", since
    an integer array cannot hold None; the caller converts back on construction.
    """
    n = len(p_samps)
    ...
    if ind_colnames:
        ...
        has_ind = ind.any(axis=1)
        first_ind = ind.argmax(axis=1)          # == first_true() for a boolean row
    else:
        has_ind = np.zeros(n, dtype=bool)
        first_ind = np.zeros(n, dtype=np.int64)

    commuter = p_samps['commuter'].to_numpy(dtype=bool)
    ...
    # One value tuple per sample row, positioned by a schema shared by the whole
    # population. Built with Python bools (not numpy.bool_) so PersonData
    # instances hold exactly what the row-wise version stored.
    ...
    if additional_traits:
        ...
        for trait in additional_traits:
            ...
            vals = col.to_numpy().astype(bool).tolist()
            cols.append([None if na else v for na, v in zip(isna, vals)])
        trait_rows = list(zip(*cols))
    else:
        trait_rows = [()] * n

    return dict(
        age=p_samps['AGEP'].to_numpy(dtype=np.int64),
        working=p_samps['has_job'].to_numpy(dtype=bool),
        commuter=commuter,
        com_cat=com_cat,
        com_inc=com_inc,
        sch_grade=sch_grade,
...
def _resolve_config(config, data_dir):
    """Deprecated alias for utils.resolve_config; kept for direct callers."""
    return resolve_config(config, data_dir)
